...
chassiss --root /path/to/project \
  --credential ~/.chassiss/credentials/orchestrator.cred \
  mission block M001 --reason blocked
...
chassiss --root /path/to/project \
  --credential ~/.chassiss/credentials/orchestrator.cred \
  task block M001-T001 --reason blocked

chassiss --root /path/to/project \
  --credential ~/.chassiss/credentials/orchestrator.cred \
  task resume M001-T001
```
...
- [Task worktree、范围与预算](09-任务工作区范围与预算.md)
- [Checks 与独立验证源](10-检查与独立验证源.md)
- [Reviewer、集成与审计](11-复核集成与审计.md)
